...
- Base URL: `$OAGE_WEBUI_URL` (default `https://open-webui.test`)
- Auth: bearer token from signin, captured into `$OAGE_WEBUI_TOKEN`

```sh
OAGE_WEBUI_TOKEN=$(curl -s "$OAGE_WEBUI_URL/api/v1/auths/signin" \
...
- Use `$OAGE_WEBUI_URL`; never hardcode the URL
